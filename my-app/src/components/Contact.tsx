...
const contactItems = [
  ...
  { icon: '📱', label: 'Phone',    value: '052-8508830',         href: 'tel:+972528508830',                                 color: '#7c3aed' },
  { icon: '💼', label: 'LinkedIn', value: 'yizik-malka',         href: 'https://www.linkedin.com/in/yizik-malka-3188a52a/', color: '#3b82f6' },
  { icon: '📍', label: 'Location', value: null,                  href: null,                                                color: '#22c55e' },
];
...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ background: 'rgba(10,22,40,0.4)', paddingBottom: '120px' }}>
      <div className="container">
        ...
        <div style={{ textAlign: 'center', marginBottom: '60px' }}>
          <p style={{ color: '#94a3b8', fontSize: '1.1rem', maxWidth: '500px', margin: '0 auto', lineHeight: 1.8 }}>
            {c.subtitle}
            <br />
            <span style={{ color: '#00f5ff' }}>{c.cta}</span>
          </p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', maxWidth: '700px', margin: '0 auto 60px' }}>
          {contactItems.map((item) => {
            const displayValue = item.label === 'Location' ? locationValue : item.value ?? '';
            return (
              <div key={item.label} style={{
                background: 'rgba(10,22,40,0.7)', border: `1px solid ${item.color}20`,
              ...
              onMouseLeave={e => {
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {item.href ? (
                    ...
                    <span style={{ color: '#e2e8f0', fontSize: '0.9rem' }}>{displayValue}</span>
                  )}
                </div>
                {!item.href && item.label !== 'Location' && (
                  <button onClick={() => copy(displayValue, item.label)} style={{
                    background: 'none', border: 'none', cursor: 'pointer',
                    color: copied === item.label ? '#22c55e' : '#475569',
                    fontSize: '0.8rem', fontFamily: "'Share Tech Mono', monospace",
                    transition: 'color 0.2s', padding: '4px',
                  }}>
                    {copied === item.label ? c.copied : c.copy}
                  </button>
                )}
              </div>
            );
      ...
      <div style={{
        marginTop: '80px', borderTop: '1px solid rgba(0,245,255,0.08)',
